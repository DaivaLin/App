...
//包圍字判斷
public class AllSurrounded extends AppCompatActivity {


    int[] animationword =new int[]{};  //動畫圖
    int[][] animationword1 = new int[][]{{}};//變化圖
    int[][] animationword2= new int[][]{{}};  //部件圖
    ImageView traditional;
    ImageView[] img = new ImageView[2];//部件圖片
    GifAnimationDrawable gif;
    float x, y;
    int mx, my; // 圖片被拖曳的X ,Y軸距離長度
    String backgroundImageName,Situation;
    ...
    //包圍字判斷
    public void AllSurrounded(View v){
        backgroundImageName = String.valueOf(v.getTag());
        if(a==0){
            if(!(mx >= 150 && mx <= 350 && my >= 115 && my <= 305)){
    ...
    private void SetStart() {
        AlertDialog.Builder builder = new AlertDialog.Builder(AllSurrounded.this);
    ...
    public void Refresh() {
        //自己调到自己的activity
        //  finish();
        Intent intent  = new Intent(AllSurrounded.this, AllSurrounded.class);
